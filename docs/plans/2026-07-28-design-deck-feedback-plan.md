...
**Status:** Implemented for new registered Design surfaces on 2026-07-29.
The rollback controls remain for the planned observation window; historical
unregistered decks are intentionally read-only rather than backfilled.
**Date:** 2026-07-28
...
- Phases 0–4 are implemented with durable contracts, the inert renderer,
  authenticated parent controller, same-thread chair resume, two-step review,
  exact provenance, and focused refinement.
- Phase 5 is cut over for newly rendered surfaces. Live legacy surfaces use the
  rollback UI or are regenerated by a new Design round; historical files are
  never made interactive retroactively.
- Phase 6 documentation is complete. Physical deletion of the rollback Design
  components remains intentionally gated on the one-release telemetry window.
