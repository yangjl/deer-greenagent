...
call. The model emits one of these *alongside* its ordinary conversational
reply; the Supervisor reads it from ``structured_response`` and passes it
...
class DbtlDiscoveryPackage(BaseModel):
    ...
    intended_outputs: list[str] = Field(
        description=(
            "1-10 concrete named artifacts or observable decisions the cycle should produce "
            "(e.g. 'fit.py', 'model.json', 'holdout_metrics.json'). Never a placeholder like "
            "'a reviewed result for ...'."
        ),
    )
