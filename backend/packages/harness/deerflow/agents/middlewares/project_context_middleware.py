...
def build_dbtl_discovery_reminder(value: object) -> str | None:
    ...
    if value.get("decision") == "keep_discussing":
        lines.append("The owner chose Keep discussing on the latest proposal. Respond as the ordinary Lead Agent: acknowledge their choice naturally, continue the substantive conversation, and ask at most one material question.")
    else:
        lines.extend(
            (
                "When — and only when — the conversation holds a concrete objective, at least one concrete intended "
                "output, and a real reproducibility/risk reason for DBTL, fill the DbtlDiscoveryPackage structured output "
                "from the whole conversation (not just the last message). Put the ordinary conversational reply that should "
                "appear before the decision card in assistant_response.",
...
class ProjectContextMiddleware(AgentMiddleware[AgentState]):
    ...
    def _with_reminder(self, request: ModelRequest) -> ModelRequest:
        ...
        discovery_evidence = render_project_discovery_evidence(discovery_context.get("evidence") if isinstance(discovery_context, dict) else None)
        blocks = [
            block
            for block in (
