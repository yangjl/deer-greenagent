...
DBTL model-use authorization: `deerflow.agents.dbtl.model_access` applies the composer path's `model:use` policy to DBTL's own model choices. `LiveStageAdapter._known_models` filters the meeting preflight card's model pickers (and, through the same list, participant-edit validation and the council-default check) via `filter_resources`; worker dispatch enforces `authorize("model", "use")` on the effective seat model immediately before the `SubagentExecutor` runs (deny falls back to the first authorized model, fail-closed with nothing allowed refuses the worker); and every setup/intent/roster/revision/assessment one-shot factory enforces the same check per call against the run's principal, threaded from `make_project_supervisor` via `principal_context(config)`. The Gateway's `draft-setup` route enforces it through `app.gateway.authz.is_model_use_authorized`, the async `aauthorize`-based helper the `get_model` route also uses; `list_models` offloads the sync `filter_resources` call to a worker thread so a blocking custom provider cannot stall the event loop. Tests: `tests/test_dbtl_model_authorization.py`, `tests/test_models_authorization.py`.
