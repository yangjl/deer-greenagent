...
**The setup questions are a native form, not a second renderer.** The DBTL
setup wizard and the upstream structured form solved the same problem in two
protocols, so `_setup_question_form_fields` projects the questions onto the
native fields instead: each `SetupQuestion` becomes a typed field whose
`default` is the model's proposal (the scientist corrects rather than composes)
and whose `description` carries the `why` plus the provenance sentence, so an
invented answer ("Suggested — correct it if wrong.") stays visually distinct
from one the request itself supports ("From your request."). Questions carrying
options become strict `select`s, which is why a trailing free-text field is
appended: a scientist must never be forced into the model's menu, and that
field is the wizard's old "Other" escape hatch. Fields are deliberately
optional — an untouched proposal is still an answer. `setup_questions` still
rides on the artifact for inspection, and a card with no projectable questions
keeps the legacy `free_text` shape, so the frontend wizard remains the
fallback rather than a parallel path.
