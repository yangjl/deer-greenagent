"""Webhook-dedupe migration regression test (issue #4120).
...
async def test_webhook_dedupe_migration_creates_composite_pk_table_and_is_idempotent(tmp_path: Path) -> None:
    db_path = tmp_path / "deer.db"
    url = f"sqlite+aiosqlite:///{db_path}"
    engine = create_async_engine(url)
    try:
        # Seed all baseline tables, then drop ONLY webhook_deliveries so the
        # webhook-dedupe upgrade actually exercises its create_table path (not the
        ...
        # Runs upgrade head -> executes the webhook-dedupe migration.
